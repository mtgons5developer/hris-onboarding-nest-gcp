...
async function consumeAuthCode(): Promise<string | null> {
  ...
  // This pool uses email aliases, so access-token `username` is a UUID. The ID token
  // carries `email` + `aud` + `cognito:groups` and is what Nest uses to link seed users.
...
export function hostedUiLogout(): void {
  ...
  url.searchParams.set('logout_uri', landingUrl());
